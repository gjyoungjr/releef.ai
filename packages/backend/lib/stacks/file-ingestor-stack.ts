...
export class FileIngestorStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: FileIngestorStackProps) {
    ...
    const storeDocMetadatQueue = new sqs.Queue(this, "StoreDocMetadataQueue", {
    ...
    fileIngestTopic.addSubscription(
      new sns_subscriptions.SqsSubscription(storeDocMetadatQueue)
    );
    fileIngestTopic.addSubscription(
    ...
    props.coreTable.grantReadWriteData(storeDocMetadata);
    this.filesBucket.grantReadWrite(storeDocMetadata);
    this.filesBucket.grantReadWrite(embedDocument);

    storeDocMetadata.addEventSource(new SqsEventSource(storeDocMetadatQueue));
